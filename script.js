// ============================================================
// BlueBee Studio — script.js
// Hasil pemisahan dari file tunggal BlueBee_Studio_2.0.html.
//
// Urutan eksekusi HARUS tetap: Bagian A -> B -> C (sama seperti
// urutan tag <script> asli). Semua kode disalin apa adanya.
//
// Catatan perilaku:
//  - Komunikasi Blok B <-> C memakai flag deterministik
//    window.__bbPendingTab (bukan timing), sehingga aman digabung
//    dalam satu file ini.
//  - Karena <script src="script.js"> diletakkan di akhir <body>,
//    seluruh elemen DOM sudah ada saat kode dieksekusi.
// ============================================================

// ============================================================
// BAGIAN A — qrcode-generator 1.4.4 (MIT)
// (aslinya embedded di <head> agar QR Generator berfungsi offline)
// ============================================================
...
// ============================================================
// BAGIAN B — Script utama aplikasi (state, tools, render, init)
// (aslinya tag <script> pertama di akhir <body>)
// ============================================================
// ============================================================
...
function toggleSidebar() {
  ...
  if (activePanel) {
    ...
    // [FIX v4.5] Append ke DALAM .studio-app (bukan document.body) supaya
    // overlay & drawer berada dalam stacking context yang sama — z-index
    // drawer (10000) dijamin di atas overlay (9998) di semua browser/HP.
    // Dulu di body: overlay selalu menutupi drawer -> layar biru + menu tak terlihat.
    (document.querySelector('.studio-app') || document.body).appendChild(overlay);
...
// ============================================================
// BAGIAN C — Landing <-> App Bridge + tool Images->PDF, PDF
// Compress, QR Generator, Lubang Binder + finalisasi
// window.__bbPendingTab (aslinya tag <script> kedua)
// ============================================================
// ============================================================
...
function renderQrGen(skipLeftPanel) {
  ...
  if (!skipLeftPanel) {
    ...
    if (st.mode === 'single') {
      ...
      createNumberInput('qrgenLeftPanel', 'Padding', st.quiet, v => { st.quiet = v; }, 1, ICONS.margin, 10, 1);
      /* [FIX v4.6] qrgenMultiCacheClear() DIHAPUS — fungsi sisa mode "Multiple"
         (dihapus sejak v4.9) yang tidak pernah didefinisikan. Setiap memilih opsi
         Koreksi Error, panggilan ini melempar ReferenceError sehingga handler klik
         berhenti sebelum menu ditutup & renderAll() dijalankan -> dropdown tampak
         tidak berubah sama sekali. Tidak ada cache QR saat ini: renderQrGen(true)
         sudah menggambar ulang preview dengan level baru (bbBulkBindRight memakai
         qrgenState.level), lalu renderAll() dari createDropdown memperbarui label. */
      createDropdown('qrgenLeftPanel', 'Koreksi Error', lvlOpts, st.level, v => { st.level = v; renderQrGen(true); }, ICONS.bleed);
